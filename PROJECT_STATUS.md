...
6. Batch upload with retry (500 rows/call) — safe to implement naively now: the Backend deduplicates a resent `batch_index` per trip (fixed 2026-08-07, `trips.received_batches` + `trip_crud.has_batch()`/`mark_batch_received()`, checked in `upload_batch` before inserting), matching `API.md`'s "safely retry after a dropped connection" guarantee. Covered by `test_duplicate_batch_index_not_double_inserted`.
...
**Why this is low-risk to start now:** the Backend's 4 endpoints are frozen, implemented, and pass an end-to-end test suite (`Backend/tests/test_api.py`, 5/5) using synthetic trips — including the retry/dedup path. Android development can proceed against a running local Backend with no further backend changes required.
...
- **Tests** — `Backend/tests/test_api.py`, 5/5 passing: full round trip, duplicate-batch-index dedup, unknown-trip 404, duplicate-trip 409, result-before-process 409.
...
**Resolved 2026-08-07:** duplicate `batch_index` deduplication (was §9 finding #3) — implemented in `trip_crud.has_batch()`/`mark_batch_received()`, checked in `upload_batch()` before inserting. `Database.md` and `API.md` now agree with the implementation.
...
5. ~~Fix `crud/sensor.py`'s missing `batch_index` deduplication~~ — **done 2026-08-07**, see §9 #3.
...
3. ~~**[MEDIUM] The frozen API contract is violated by the implementation.**~~ **RESOLVED 2026-08-07.** `Documentation/API.md`'s guarantee — *"Duplicate batch_index values are ignored rather than double-inserted, so the app can safely retry after a dropped connection"* — previously did not match `Backend/app/crud/sensor.py`, whose docstring admitted resent batches were not deduplicated. Assessed against the frozen contract: dedup was judged required (not a doc-only fix), since the planned Android client explicitly needs retry-after-dropped-connection to be safe, and real mobile networks make "server committed, client never saw the response" a routine failure mode, not an edge case. Implemented via `trips.received_batches` (new column, documented in `Database.md`) + `trip_crud.has_batch()` / `mark_batch_received()`, checked in `upload_batch()` before inserting. Regression test: `test_duplicate_batch_index_not_double_inserted`.
...
2. ~~**Close the two concrete Backend bugs (~1–2 hours):** fix (or formally waive) `batch_index` deduplication~~ — **done 2026-08-07.** Still open: retire or fix `step10_global_shap.py`'s axis bug and correct `ML/README.md`'s run order.
